...
When the client instance makes its <Link to="/transactionrequest">initial request</Link> or a <Link to="/continue">continuation request</Link>, the AS responds with a JSON structure that contains details about what the client instance can do next. This includes some combination of the following: 
