...
The response from the grant endpoint tells the client instance what it needs to do next, including if it needs to <Link to="/interaction">interact with the user</Link> and any <Link to="/tokens">tokens that have been issued</Link>. 

If the transaction can be continued by the client instance, the AS includes a `continue` section in the response as well.

The response can also include handles that the client instance can use in future requests to refer to itself or the current user by reference.
