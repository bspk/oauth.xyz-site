...
The client credentials grant in GNAP is simply a request that does not require user interaction, nor does it require any information about the user from the client. In other words, neither a `user` nor `interact` section are included in the <Link to="/transactionrequest">request</Link>. 
