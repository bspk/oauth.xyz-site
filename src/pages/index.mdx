...
XYZ has several core features that drive its design principles, and these are key to providing a consistent data model:
 ...
 - Intent registration. This allows the client to start the process off the same way every time by maing a <Link to="/transactionrequest/">request to the authorization server</Link>.
