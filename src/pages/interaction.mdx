...
During a transaction, the AS will often require interaction from a user. This user can be the same person running the client software, or it could be another party who's not necessarily available. In the request, the client tells the AS how it is able to interact with the user. Several modes of interaction are defined, and a client can support multiple forms of interaction simultaneously.

 - The `redirect` and `short_redirect` capability is used when the client is capable of opening a browser on the same device for the user to interact with the AS, or otherwise directing the user to an arbitrary URL.
 - The `user_code` capability is used when the client can communicate an out-of-band code for the user to provide to the AS.
 
In addition, the client can indicate how it can receive responses from the interaction request. Currently, there are two modes of getting a response defined and they can be declared independently:

 - The client can set a `callback` capability including a URL that the AS can send the user back to when done interacting
 - The client can set a `pushback` capability including a URL that the AS can send an HTTP message to when the user is done interacting

The client can poll the AS using a continuation handle as well.

These can be used independently of other interaction capabilities, but it will likely be most common to use a callback with the redirect method, polling with the user code capability, and a few others. Common combinations are shown here.
...
When the AS has collected whatever input it needs from the user, it sends the user back to the client with an HTTP redirect. First, the AS creates a unique, unguessable interaction handle that the client can use in its subsequent interaction request. The AS also calculates a `hash` to allow the client to confirm the transaction by concatenating the following values with a newline `\n` character and computing the hash based on the `hash_method` in the client's initial transaction request (defaults to `SHA3` 512-bit):
 ...
 - the `callback_server_nonce` value returned in the initial transaction response
 - the unique interaction reference
...
In extensions to the protocol, some clients will be able to pass a message from the AS to another piece of software using a secondary protocol, such as DIDComm. 
...
In either case, the agent might send its response directly to the AS through some external means, or the client might get a message back from the agent. This message is to be included in its continuation request.
