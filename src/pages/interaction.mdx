...
By default, the callback return will use a `SHA3` 512-bit hash. The client can choose a different hash algorithm by sending a `hashMethod` parameter. In this example, a `SHA2` 512-bit hash is requested. If the AS doesn't support the client's requested hash method, it returns an error.
