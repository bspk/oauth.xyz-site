...
Clients in XYZ are always identified by their key. This key can be presented as a value or as a reference. The key binds the client during the grant process and must be proved for all requests. The AS can restrict access to certain keys, associated with particular clients and controlled through a registration process. The AS can also allow clients to dynamically present and prove a key at runtime, such as a key generated on a client device and bound to only that instance of the client software on that device. 
...
If a client has a reference to its key that is known to the AS, the client can present that reference in lieu of the key itself.
...
OAuth's [Demonstration of Proof of Posession (DPoP)](https://tools.ietf.org/html/draft-ietf-oauth-dpop) provides a way to prove possession of a key through signing a header value using JWS. 
...
OAuth's Proof of Possession token architecture also defined a [mechanism for signing HTTP messages](https://tools.ietf.org/html/draft-ietf-oauth-signed-http-request) in a draft that has since expired. However, the signature mechanism is still valid, and it works like a mix of the Cavage and DPoP methods.
