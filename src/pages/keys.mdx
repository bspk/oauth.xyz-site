...
The key in this method is presented as a JSON Web Key Set by the client during the transaction request.
...
> We could also do cavage-signatures or a number of other methods
...
> I'm interested to see how we'd do this in a practical sense, but I haven't seen a lot of work on signing HTTP with ZKP algorithms, only highly specialized queries. The keys retrieved from a `did` could be in any number of formats, and could be presented and proofed in several different ways. 
